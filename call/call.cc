...
Call* Call::Create(const Call::Config& config,
  ...
  return new internal::Call(
      clock, config,
      /*RtpTransportControllerSend 是 WebRTC 中负责发送侧 RTP 流量控制的核心模块。它是 RTP 传输的控制器，
        负责管理和协调发送侧的各种功能，包括比特率管理（带宽估算 BWE）、节奏控制（Pacer）、网络状态预测和反馈处理等
      */
...
void Call::UpdateAggregateNetworkState() {
  ...
  // 通知 RtpTransportControllerSend 网络可用
