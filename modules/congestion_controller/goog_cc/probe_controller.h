...
/**
 * 主要负责生成 ProbeClusterConfig ，也就是探测源信号或者说判断是否需要探测。
 * 是整个探测逻辑的管理器，场景分类如下：
 * 1.初始阶段，需要进行带宽探测；
 * 2.应用编码设置画质码率约束更改，向拥塞控制框架allocate申请码率，会触发探测；
 * 3. GoogCcNetworkController 模块在综合基于"delay_based、lossbased、以及ack码率"后，
 *    会设置目标估计码率到 ProbeController，此时也有可能会出发带宽探测；
 * 4.过载恢复后会触发探测；
 * 5.定时，周期性探测，默认是25ms；
 * 
 * 以上场景，如果逻辑上需要探测，则会在 ProbeController 模块生成 ProbeClusterConfig 探测源数据。
 */
