...
std::vector<ProbeClusterConfig> ProbeController::SetBitrates(
  ...
  // 默认情况下max_bitrate不会被设置为-1，所以最大探测带宽为5Mbps(kDefaultMaxProbingBitrate)
  ...
  switch (state_) {
    case State::kInit:
      if (network_available_)
        return InitiateExponentialProbing(at_time_ms); // 从初始状态启动指数探测
      break;
 ...
 * @param bitrates_to_probe：待探测的目标比特率列表
...
std::vector<ProbeClusterConfig> ProbeController::InitiateProbing(
    ...
    bool probe_further) {
  /**
   * 1）新特性，确保目标码率在瓶颈链路带宽或者当前估计的码率水位线以下：
   * 期望的最大探测带宽是 max_probe_bitrate_bps , 
   * 最大估计链路带宽为 std::min(max_probe_bitrate_bps, max_total_allocated_bitrate_ * 2);
   * 
   * 当前估计的带宽
   */
  ...
  if (limit_probes_with_allocateable_rate_ &&
    ...
    // 如果已经配置了最大码率，允许探测速率是最大码率的2倍
    // 1.这是为了多一些开销来应对突发流
    // 2.还可以避免由于探测码率经常低于目标码率，导致的质量轻微下降
